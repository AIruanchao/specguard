<!-- 
修正版V2: 
- 系统口径: 6→7(含SpecGuard cloud3只读)
- 工期: 10天→14天(含2天缓冲)
- 新增: Day10-12缓冲+第7系统同步+稳定性修复+验收报告
- 新增: Day13-14 P1准备
-->

以下按 **14 个工作日（含2天缓冲）、6 个系统并行推进**设计。默认使用 GitHub Actions；若实际为 GitLab/Jenkins，仅替换 CI 模板载体，任务和验收口径不变。

统一约定：

- 系统 ID：`business-api`、`specguard`、`dh-factory`、`erp-web`、`stock-api`、`ledger`、`specguard-cloud3`

- **7个系统**（含SpecGuard cloud3生产部署，只读同步SDD配置，不参与CI构建）
